...
import { AddCartItemSchema, UpdateCartItemSchema } from "../types/cart.types.js";
...
export const updateCartItem = async (req: AuthRequest, res: Response) => {
  ...
  const updatedCartItem = await cartService.updateCartItem(user.id, cartItemToUpdate)
  res.status(200).json({
...
export const deleteCartItem  = async (req: AuthRequest, res: Response) => {
  const {id} = req.params;
  const deletedCartItem = await cartService.deleteCartItemById(id.toString());
    res.status(200).json({
